...
Este es el valor de $\bar{x}$, por encima del cuál se encuentra sólo el 2.5\% de los valores de $\bar{X}$ cuando $\lambda$ es por ejemplo 0.2 

```{r}
xbar0.975(lambda=0.2)
```

Miremos varios valores de $\lambda$ y calculemos cuando $xbar0.975$ es precisamente $xbar$
...
lambdalow
```
Asumiendo ahora estos valores estimados de $\lambda$, que llamaremos $\hat{\lambda}_{low}$, hemos tenido mala suerte. Es decir que el trabajador realmente se encontraría en una zona bastante segura, sólo que $\bar{x}=0.5$ resultó en ser excepcionalmente alto. Así mismo podemos suponer valores de $\lambda$ para los que hemos tenido mucha suerte y hemos detectado muy pocas partículas en una situación de exptrema contaminación. Para esto calculamos el cuantíl al 2.5\% de una distribución de los valores dados por *Xbar10*, y vemos cuando coincide con $\bar{x}=0.5$  
...
```{r}
distL <- sapply(rep(10,10000), Xbar10, lambda=lambdalow[1])
quantile(distL,0.975)

hist(distL, freq=FALSE, breaks=seq(0,3,0.025), main="", col="red")

distU <- sapply(rep(10,10000), Xbar10, lambda=lambdahigh[7])
...
los valores estimados $\hat{\lambda}_{low}$ y $\hat{\lambda}_{high}$ definen un intervalo, que se puede entender como el resultado de un intervalo aleatorio que captura a $\lambda$ el $95\%$ de las veces. Este es un intervalo de confianza del 95\%.
