...
Primero hagamos una gran consideracón. Nuestras mediciones $x_1,..x_n$, o conteos, son eventos independientes y que provienen de un proceso común, que puede ser descrito por una única variable aleatoria $X$. La función de probabilidad de $X$, $f_X$, la llamamos **poblacional**, por el hecho de dar probabilidad a cada uno de los resultado posibles de una medición. Como en el caso de un censo, las mediciones no pueden ser separadas de los individuos que representan, entonces la distribución de todas las posibles medidas que se en la población. Es una función abstracta y tiene la libertad de representar idividuos que todavía no existen o, partículas radiactivas de átomos que todaía no ha decaído. Aún así suponemos que el proceso de craeción que representa existe y que lo podemos conocer. 

La suposición de medidas independientes tampoco está siempre garantizado ya sea porque el conteo de partículas en un momento dependa de cuantas se detectan previamente, o porque los individuos estén relacionados y no representen a la población. Sin embargo, no tenemos razones para pensar que en nuestro caso esto no sea cierto. Por lo que nos vamos a permitir suponer un **modelo** de probabillidad para los datos $x_i$. Es decir, queremos derivar unas características generales de cómo se produce la variabilidad en los conteos; por qué ahora uno, después ninguno y tal vez en 10 sgundos dos. El objetivo es usar la forma de $f_X$ para deducir las posibles formas que puede tomar $f_{\bar{X}}$. 

Para modelar $f_{X}$ solemos empezar considerando una circunstancia para la cual nuestra ignorancia sobre los eventos es razonablemente total. De este punto de patida construimos la situación que queremos describir. Imaginemos que tuviésemos un contador que pudiese detectar partículas en milésimas de segúndo. Seguramente se en un milisegundo se detectíamos una o zero partículas (no se detectarían más de una). No es mucho pedirle a nuestro proceso radiactivo. Si es así entonces detectaríamos una partícula con una probabilidad que sería igual al número promedio de detecciones por segundo que llamamos $\lambda$, dividido por 1,000. $\lambda$ es ideal, una característica del proceso imaginado. Volviendo al mundo de nuestro contador Geiger real, si contamos todas las partículas detectadas en un segundo estos son $x$ partículas detectadas en 1,000 ensayos, uno por cada milisegundo. Este tipo de conteo probabilístico sigue una distribución binomial, que no describiré, pero que nos sirve para entender que es la base para obtener la distribución de Poisson       
\[
    f_{X}=f(x; \lambda)=P(X=x)= \frac{\lambda^x e^{-\lambda}}{x!} 
\]
cuando imaginamos contadores infinitamente rápidos, en tiempo de detección 0 para acegurarnos que si bien varias partículas se pueden contar en un intervalo de tiempo, estas no se crean exactamente al mismo tiempo. Suponemos así que el proceso las crea una a una y no dos a la vez y con cierta cadencia $\lambda$. El intervalo en milisegunodos era todavía una aproximación, útil para anclarnos en un proceso binomial, pero insuficente para destilar la idea de que cada partícula es independiente entre sí. Matemáticamente hablando, una situación tan razonable como esta, se ecuentra en el límite ($t \rightarrow 0$); algo que sólo una mente prodigiosa dar a luz. La distribución de Poisson sirve para modelar cualquier proceso con el cual podamos recorrer el mismo camino de supociciones. Nosotros lo usaremos para modelar la probabilidad de contar $x$ partículas radioactivas con una medía de detección por segundo $\lambda$.   

Ahora que tenemos un modelo para $f_X$ estudiaremos *teóricamente* cómo se comportaría $\bar{X}$ bajo una distribucion de Poissson para $f_X$. Más especificamente nos preguntamos ¿Cómo sería la distribución de valores de $\bar{x}$ cuando repetimos muchas veces el experimento de contar el número de partículas readioactivas por segundo durante 10 segundos y calculamos la media? Recoredemos que queremos tener una idea de en qué medida podemos confiar en las medidas de los primeros 10 segundos $\bar{x}$, si por ejemplo pudiéramos reperir la entrada del trabajador un sin fin de veces.
