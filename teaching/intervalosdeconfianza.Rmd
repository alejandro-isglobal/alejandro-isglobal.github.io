...
Veamos el intervalo de confianza para la varianza. Así como estimamos el parámetro $\mu$ de la distribución para $X$, también estimamos su varianza $\sigma^2_X$ de los datos. De hecho, hemos ultizado el valor $s$ para estimar $\sigma^2_X$ en el cálculo de los intervalos de confianza cuando $\sigma_X$ es desconocido.   
...
es un valor de la variable aleatoria $$S^2= \frac{1}{n-1}\Sigma_i (X_i-\mu)^2$$.

Si $X$ se distribye normalmente $N(\mu, \sigma_X)$ entonces se pude mostrar que la variable $S^2(n-1)/\sigma^2$ se distribuye como una $\chi^2$ con n-1 grados de libertad,  
$$\frac{S^2}{\sigma^2}(n-1)\rightarrow \chi^2_{n-1}$$
Por lo tanto buscamos $f_{inf}$ y $f_{sup}$
$$P(f_{inf}\leq \frac{S^2}{\sigma^2}(n-1) \leq f_{sup})=0.95$$
Por lo tanto $f_{inf}$ y $f_{sup}$ son los quantiles al 2.5\% Y 97.5\% de esta variable

$$P(\chi^2_{0.025,n-1} \leq \frac{S^2}{\sigma^2}(n-1) \leq \chi^2_{0.975,n-1})=0.95$$
En términos del intervalo aleaorio $(L,U)$ tal que $P(L\leq\sigma^2\leq U)=0.95$ tenemos

$$P(\frac{S^2 (n-1)}{\chi^2_{0.975,n-1}}\leq \sigma^2 \leq \frac{S^2(n-1)}{\chi^2_{0.025,n-1}})=0.95 $$
...
Que R se calcula con **sigma.test**
```{r}
