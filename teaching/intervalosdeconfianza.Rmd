...
A las 10 mediciones de impedancia en el histograma abajo corresponden un intervalo de confianza para $\mu$ que define dos distribuciones límite (roja y azul) que atrapan con una confianza de 95\% la distribución de $X$.  

```{r}
x <- seq(-0.5,8.5,0.05)

hist(impedancia, br=seq(-0.5,8.5),freq=FALSE)

lines(x,dnorm(x, mean=CI[1], sd=1.4), col="red")
lines(x,dnorm(x, mean=CI[2], sd=1.4), col="blue")

legend("topleft",c("ditribucion N(CIinf,1.4)", "ditribucion N(CIsup,1.4)"), col=c("blue","red"), lty=1 )
```
...
legend("topleft",c("ditribucion N(xbar,CIinf)", "ditribucion N(xbar,CIsup)"), col=c("blue","red"), lty=1 )
```
