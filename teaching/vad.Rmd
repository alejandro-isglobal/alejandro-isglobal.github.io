...
1. Sea siempre positiva para cualquier valor de $x_i$: $f(x_i)=P(X=x_i)$. No concebimos frecuencias negativas ni que sus límites lo sean.
...
$$f(k)=(1-p)^{1-k} p^k $$
para $k=(0,1)$ en donde $0$ se asigna al resultado de tipo B y $1$ al de tipo A. 
El lanzamiendo de una moneda (0:cara, 1:cruz) es un ensayo de Bernoulli con $p=1/2$. Dos características importantes de las funciones de probabilidad son su valor medio y su varianza. La media de la distribución es definida como 
$$E(X)=\mu=\sum_{i=1}^{n}f(x_i)x_{i}$$
y nos da su centro gravedad; como si en cada distancia $x_i$ huiese un punto con masa $f(x_i)$. La varianza es la distancia cuadrática media de cada uno de los $x_i$ a la media 

$$V(X)=\sum_{i=1}^{n}(x_i-\mu)^2 f(x_i)$$
es decir su momento de inercia que indica que tan disperdad estén las masas al rededor del centro de gravedad.  Para la distribución de Bernulli la media y la varianza son $$E(k)=\mu=p$$ $$V(k)=\sigma^2=p(1-p).$$ Cabe observar que la media no es necesariamente un valor posible del experimento aleatorio es tan sólo un valor que indica el punto medio donde se balancean las cargas de probabilidad.  
...
y traducir la certeza matemática de $f(k)$ en azar. Esto lo hacemos con un generador de números aleatorios o un **simulador**. La naturaleza nos provee de varios simuladores (monedas, dados, decaimientos nucleares, etc..). Sin embargo, los matemáticos han diseñado fórmulas que podemos usar para producir séries de números aletorios. La función **sample** es un generador de números aleatorio implementado en R, el que configuramos para que tome un valor aletorio entre (0,1) con probabilidad (0.8,0.2); es decir para que haga un ensayo de Bernoulli. Asignemmos *k* el vector $(0,1)$ y a *fk* el vector $(0.8,0.2)$ y simulemos el error al transmitir un sólo pixel
```{r}
k <- c(0,1)
fk <- c(0.8,0.2)
names(fk) <- k
...
Vemos como la distribución binomial negativa describe en el ĺimite de infinitas repeticiiones ($N \rightarrow \infty$) el número de éxitos en sucesivos hasta enconstrar el $r$-ésimo ($r=4$) fallo.   
