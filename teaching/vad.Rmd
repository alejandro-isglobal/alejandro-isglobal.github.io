...
es decir su momento de inercia que indica cuanto de dispersos son los valores $x_i$.  Para la distribución de Bernulli la media y la varianza son $$E(k)=\mu=p$$ $$V(k)=\sigma^2=p(1-p).$$ La función de probabilidad de Bernoulli es un modelo de probabilidad que define realmente una familia de funciones. Cada función es totalmente definida por el número, o parámtro, $p$. Este número es sucifiente para calcular $f(0)$ y $f(1)$ y por lo tanto $E(k)$ y $V(k)$.  
...
¿Cómo podemos reproducir entonces el resultado de un experimento aleatorio?. Queremos usar $f(k)$ para recrear un experimento

$$f(k) \Longrightarrow0, 1, 0, 0, 0, 1, ...$$ 

y traducir su certeza matemática en azar. Esto lo hacemos con un generador de números aleatorios o un **simulador**. La función **sample** es un generador de números aleatorio implmentado en R, el cual lo conficuramos para que tome 1 valor aletorio entre (0,1) con probabilidad (0.8,0.2); es decir para que haga un ensayo de Bernoulli. Asignemmos *k* el vector $(0,1)$ y a *fk* el vector $(0.8,0.2)$ y simulemos el error al trasmitir un sólo pixel
```{r}
k <- c(0,1)
fk <- c(0.8,0.2)
names(fk) <- k
...
Simulemos la transmisión de una foto de 100 píxeles, todos blancos. Es decir, transmitamos 100 pixels cada uno con una probabilidad de 0.2 de ser transmitido erroneamente. Los trasmitidos con error se verán negros en la foto final. Para esto sólo tenemos que incrementar el número de ensayos de Bernoulli en **sample** 
```{r}
