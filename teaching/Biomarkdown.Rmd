...
detected with the biomarker (positive)?
...
**Sensitivity**: $fr_{[cutoff=-1]}(positive|yes)$
...
**False positive rate** ( $1 - specificity$ ): $fr_{[cutoff=-1]}(positive|no)$
...
</br>=(*false positive rate*, *true positive rate*)$_{cut-off}$=$(0.28, 0.88)_{-1}$
  
```{r, message=FALSE, out.width = '50%'}
out <- cvAUC(hiv$test, hiv$response) #compute ROC
plot(out$perf, col="blue", main="ROC") #plot
lines(c(0,1),c(0,1)); points(0.28, 0.88, pch=16) #cutoff=-0.5
```
...
plot(out$perf, col="blue", main="ROC") #plot
lines(c(0,1),c(0,1)); 
points(0.28, 0.88, pch=16)#cutoff=-0.5
