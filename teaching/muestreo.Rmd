...
Primero hagamos una gran consideracón. Nuestras mediciones $x_1,..x_n$, o conteos, son eventos independientes y que provienen de un proceso común, que puede ser descrito por una única variable aleatoria $X$. La función de probabilidad de $X$, $f_X$, la llamaremos **poblacional**, por el hecho de darnos las probabilidades de obtener cada uno de los resultado posibles que resulten de una medición. Usamos en término poblacional inspirados en un censo, donde las mediciones no pueden ser separadas de los individuos que representan y $f_x$ da la probabilidad a todas y cada uno de los posibles varoles de $X$ en la población. $f_x$ es abstracta y tiene la libertad de representar idividuos que todavía no existen o, partículas radiactivas de átomos que todaía no ha decaído. Aún así suponemos que el proceso de craeción que representa existe y que lo podemos conocer, o como mínimo modelar. 

La suposición de que $x_1,..x_n$ son independientes no está siempre garantizada, ya sea porque el conteo de partículas en un momento dependa de cuantas se detectan previamente, o porque los individuos en un censo estén relacionados y no representen a la población. Sin embargo, no tenemos razones para pensar que en nuestro caso, esto sea el caso. Nos vamos a permitir entonces suponer un **modelo** de probabillidad para los dato $x_i$ independientes. Queremos derivar características generales de cómo se producen diferentes conteos en el detector segundo a segundo por diez segundos; por qué ahora uno, después ninguno y tal vez en 10 segundos dos. El objetivo es usar la forma de $f_X$ para deducir las posibles formas que puede tomar $f_{\bar{X}}$. 

Para modelar $f_{X}$ solemos empezar considerando una circunstancia para la cual nuestra ignorancia sobre los eventos es razonablemente total. De este punto de patida construimos la situación que queremos describir. Imaginemos que tuviésemos un contador que pudiese detectar partículas en milésimas de segúndo. Seguramente en un milisegundo detectíamos o una o cero partículas: no se detectarían más de una. No es mucho pedirle a nuestro proceso radiactivo. Si es así entonces detectaríamos una partícula con una probabilidad que sería igual al número promedio de detecciones por segundo que llamamos $\lambda$. $\lambda$ es ideal, una característica del proceso imaginado. Con esta perspectiva, volviendo al mundo de nuestro contador Geiger real, si contamos todas las partículas detectadas en un segundo estos son $x$ partículas detectadas en 1,000 ensayos, uno por cada milisegundo. Este tipo de conteo probabilístico sigue una distribución binomial, que describo en otro tutorial, pero que aquí nos sirve para decir que es una buena posición para entender que la distribución de Poisson       
\[
    f_{X}=f(x; \lambda)=P(X=x)= \frac{\lambda^x e^{-\lambda}}{x!} 
\]
surge cuando imaginamos contadores infinitamente rápidos, en tiempo de detección de 0 segundos, o cuando el número de ensayos en la distribución binomial crece a infinito. El intervalo en milisegunodos era todavía una aproximación, útil para anclarnos en un proceso binomial, pero insuficente para destilar la idea de que las partículas son independientes entre sí. Este no es un proceso, como la emisión de un par electrón-positrón, que genere múltiples partículas a la vez. En el límite de rapidez, nos acegurarnos que las partículas no se crean exactamente al mismo tiempo, es decir que el proceso las crea de una a una y con cierta cadencia $\lambda$. Pensemos otra vez en nuestro trabajador. Las partículas que él detecta son de un átomo que decae aquí y otro que decae allá. El proceso que queremos describir no es otro que la densidad de partículas radiactivas de dónde se encuentra el trabajador y $\lambda$ es una medida de esa densidad. Recordemos que nuestro objetivo es describir la peligrocidad del lugar. 

La distribución de Poisson sirve para modelar cualquier proceso con el cual podamos recorrer el mismo camino de supociciones. Nosotros lo usaremos para modelar la probabilidad de contar $x$ partículas radioactivas con una media de detección por segundo $\lambda$.   

###Modelización de la probabilidad de la media
Ahora que tenemos un modelo para $f_X$ estudiaremos *teóricamente* cómo se comportaría $\bar{X}$ bajo una distribucion de Poissson para la distribución poblacional. Más especificamente nos preguntamos ¿Cómo sería la distribución de valores de $\bar{x}$ cuando repetimos muchas veces el experimento de contar el número de partículas readioactivas por segundo durante 10 segundos y calculamos la media? Recoredemos que queremos tener una idea de en qué medida podemos confiar en las medidas de los primeros 10 segundos $\bar{x}$, si por ejemplo pudiéramos reperir la entrada del trabajador un sin fin de veces.
